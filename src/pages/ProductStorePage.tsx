...
export default function ProductStorePage({
  ...
  return (
    <>
          ...
          <div className="grid grid-cols-2 sm:grid-cols-3 md:flex md:flex-row md:justify-center gap-3 sm:gap-5 md:gap-8">
            {CATEGORIES.map((cat, index) => {
              ...
              return (
                ...
                >
                  <div className={`relative w-full aspect-square md:w-32 md:h-32 rounded-[24px] sm:rounded-[30px] overflow-hidden bg-white border border-gray-200/70 transition-all duration-700 shadow-[0_20px_45px_rgba(15,23,42,0.12)] ${isActive ? 'scale-105 shadow-2xl ring-4 ring-merlin-blue/20' : 'grayscale md:group-hover:grayscale-0 md:group-hover:scale-105'}`}>
                    ...
                    <div className={`absolute inset-0 bg-gradient-to-br from-white/0 to-gray-100/40 transition-opacity duration-300 ${isActive ? 'opacity-0' : 'md:group-hover:opacity-0'}`} />
                  </div>
                  <span className={`text-[11px] sm:text-sm font-black transition-colors duration-300 max-w-[190px] text-center leading-tight tracking-[0.02em] ${isActive ? 'text-merlin-blue' : 'text-gray-600 md:group-hover:text-black'}`}>
      ...
      <section className="px-6 py-8 md:py-10 max-w-7xl mx-auto">
        <AnimatePresence mode="wait">
          <motion.div key={activeCategory} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }} transition={{ duration: 0.5 }} className="text-center">
            ...
            <p className="text-base md:text-xl text-gray-500 max-w-2xl mx-auto mb-6 font-medium">
      ...
      <section ref={productsRef} className="py-8 md:py-10 bg-white scroll-mt-24">
